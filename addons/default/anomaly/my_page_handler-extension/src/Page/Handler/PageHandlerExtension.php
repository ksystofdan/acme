<?php namespace Anomaly\MyPageHandlerExtension\Page\Handler;
            ...
            return implode(
                ...
                $translations->map(
                    function ($translation) use ($page) {
                        ...
                        $translationPath = str_replace( ['de/', 'en/', 'ar/', 'es/'] ,"", $translationPath);
                        return "Route::any('{$translationPath}', [
            ...
            return implode(
                ...
                $translations->map(
                    function ($translation) use ($page) {
                        ...
                        $translationPath = $translation->path;
                        $translationPath = str_replace( ['de/', 'en/', 'ar/', 'es/'] ,"", $translationPath);
                        return "Route::any('{$translationPath}/{any?}', [
    'uses'                       => 'Anomaly\\PagesModule\\Http\\Controller\\PagesController@view',
    'as'                         => 'pages::{$page->getId()}.{$translation->locale}',
    'streams::addon'             => 'anomaly.module.pages',
    'anomaly.module.pages::page' => {$page->getId()},
